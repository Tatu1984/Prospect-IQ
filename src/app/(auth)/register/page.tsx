  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await fetch("/api/register", {
        ...
        body: JSON.stringify({ name, email, password, purpose }),
      });
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          type="button"
          className="flex items-center justify-center gap-2 rounded-lg border border-border bg-card p-2.5 transition-colors hover:bg-muted opacity-50 cursor-not-allowed"
          disabled
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium" htmlFor="purpose">
            How will you use ProspectIQ?
          </label>
          <div className="relative">
            ...
            <select
              ...
              className="w-full appearance-none rounded-lg border border-input bg-background py-2.5 pl-10 pr-4 text-sm outline-none transition-colors focus:border-primary focus:ring-2 focus:ring-ring/20"
              required
            >
        ...
        <div className="flex items-start gap-2">
          ...
          <label htmlFor="terms" className="text-xs text-muted-foreground">
            ...
            I understand ProspectIQ aggregates publicly available data and will not use
